...
num_atom_type = 119+1 # including the extra mask tokens. last for mask
num_chirality_tag = 4+1 
...
class GINConv(MessagePassing):
    def __init__(self, emb_dim, aggr="add"):
        ...
        self.edge_embedding3 = nn.Embedding(num_bond_direction, emb_dim)
...
class GNN(nn.Module):
    def __init__(self, num_layer = 3, emb_dim = 128, JK="last", drop_ratio=0.1, gnn_type="gin"): # 5
        if num_layer < 2:
    ...
    def forward(self, *argv):
        ...
        return node_representation, h0, h_list


class GNN_graphpred(nn.Module):
    def __init__(self,  num_tasks, molecule_model=None):
    ...
    def from_pretrained(self, model_file):
        self.molecule_model.load_state_dict(torch.load(model_file)['model_2d'])
        return
